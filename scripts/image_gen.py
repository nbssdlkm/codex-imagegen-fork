...
from _config import load_credentials  # noqa: E402
...
def _ensure_api_key(dry_run: bool) -> None:
    """Load credentials via _config.load_credentials() (4-path fallback chain).

    Fallback order (see _config.py):
      0. OPENAI_API_KEY env (+ optional OPENAI_BASE_URL)
      1. EPHONE_API_KEY env -> auto redirect to ephone
      2. ~/.config/codex-imagegen-fork/config.toml
      3. RuntimeError -> agent should trigger SKILL.md First-time setup

    Sets OPENAI_API_KEY + OPENAI_BASE_URL env so the OpenAI SDK picks them up
    at client init (the SDK lazy-reads env when constructing OpenAI()/AsyncOpenAI()).
    """
    try:
    ...
    print(f"API key loaded; base_url={base_url}", file=sys.stderr)
...
def _generate(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # 2026-05-13: surface API errors loudly instead of silent crash
        # (GLM diagnosed: standard CLI was crashing with exit 1 + no stderr because
        # exceptions bubbled past argparse main and got swallowed by subprocess wrappers)
        import traceback
        print(f"\nERROR: client.images.generate() failed: {type(e).__name__}: {e}", file=sys.stderr)
        print(f"  base_url={os.environ.get('OPENAI_BASE_URL', '(SDK default)')}", file=sys.stderr)
        print(f"  model={payload.get('model')}, size={payload.get('size')}, quality={payload.get('quality')}", file=sys.stderr)
        if hasattr(e, 'response'):
            print(f"  http_status={getattr(e.response, 'status_code', '?')}", file=sys.stderr)
            body_text = getattr(e.response, 'text', '?')
            print(f"  http_body={body_text[:500] if isinstance(body_text, str) else body_text}", file=sys.stderr)
...
def _edit(args: argparse.Namespace) -> None:
    ...
    with _open_files(image_paths) as image_files, _open_mask(mask_path) as mask_file:
        ...
        try:
            ...
            # exceptions bubbled past argparse main and got swallowed by subprocess wrappers)
            import traceback
            print(f"\nERROR: client.images.edit() failed: {type(e).__name__}: {e}", file=sys.stderr)
            print(f"  base_url={os.environ.get('OPENAI_BASE_URL', '(SDK default)')}", file=sys.stderr)
            print(f"  model={payload.get('model')}, size={payload.get('size')}, quality={payload.get('quality')}", file=sys.stderr)
            print(f"  n_images_input={len(image_paths)}, mask={'yes' if mask_file else 'no'}", file=sys.stderr)
            if hasattr(e, 'response'):
                print(f"  http_status={getattr(e.response, 'status_code', '?')}", file=sys.stderr)
                body_text = getattr(e.response, 'text', '?')
                print(f"  http_body={body_text[:500] if isinstance(body_text, str) else body_text}", file=sys.stderr)
